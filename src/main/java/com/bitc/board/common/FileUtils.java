...
//        업로드된 파일 정보에서 모든 파일 이름을 가져옴
        ...
        String newFileName; // 새로운 파일 이름 설정
        String originalFileExtension; // 확장자를 저장하기 위한 변수
        String contentType; // 파일의 타입을 저장하기 위한 변수
...
//                    nanoTime() : 자바 언어에서 현재 시간을 기준으로 1/1000초까지 계산된 시간정보를 가져옴
//                    서버에 파일을 저장 시 동일한 이름의 파일을 저장할 수 없으므로 현재 시간을 기준으로 파일 이름을 변경하여 저장
//                    동일한 시간에 서버에 접속하여 파일을 업로드하는 경우가 발생할 수 있으므로 최대한 파일 이름의 중복을 피하기 위하여 사용
...
//                    파일 정보를 저장하기 위한 BoardFileDto 클래스 타입의 객체 생성
                    ...
                    boardFile.setStoredFilePath(path + "/" + newFileName); // 위에서 생성한 파일 경로와 새로 생성한 파일이름을 합쳐서 저장
